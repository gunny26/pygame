...
from FastMath import mandelbrot_complex, mandelbrot_noncomplex
...
class MandelbrotZoom:
    ...
    def __init__(self, dim: tuple, middle: complex, magnify: float, maxiter: int = 100, palette: list=PALETTE):
        ...
        self.scale = 2  # scale of smaller calculation surface
        self.surface = pygame.Surface((dim[0] // self.scale, dim[1] // self.scale))
        # zooming constants
        self.step = 0.04375
        self.framecount = 0

    def update_c_complex(self) -> pygame.Surface:
        """
        ...
        """
        data = mandelbrot(self.middle, self.step.real, self.surface.get_width(), self.surface.get_height(), int(self.maxiter))
        color = pygame.PixelArray(self.surface)
        for index, point in enumerate(data):
            x = index % self.surface.get_width()
            y =  index // self.surface.get_width()
            color[x, y] = self.palette[point % 256]
        color.close()
        self.step *= self.magnify  # zoom in
        self.maxiter = self.maxiter * 1.01
        self.framecount += 1
