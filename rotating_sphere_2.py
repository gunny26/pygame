...
class Tree(object):
    ...
    def draw(self, root, angle, length, width):
        ...
        pygame.draw.line(self.surface, self.color, root, dest, width)
    ...
    def tree(self, depth, length, root, angle):
        if depth == 0:
            if self.leafs is True:
                ...
                pygame.draw.ellipse(self.surface, leaf_color, (int(root.x), int(root.y), 10, 10), 0)
            return
...
class Circle(object):
    ...
    def __init__(self, surface, color, center=Vec3d(0, 0, 0), size=Vec3d(1, 1, 1), steps=36.0):
        self.surface = surface
        self.color = color
        self.size = size
        self.center = center
